...
train_label = [train_label; label_tem ];
	  ...
	  mn = mean(train_set);
train_set = bsxfun(@minus,train_set,mn); % substract mean
[coefs,scores,variances] = princomp(train_set,'econ'); % PCA
dims = 9;
train_set = train_set*coefs(:,1:dims); % dims - keep this many dimensions
...
test_label = [test_label; label_tem ];
	  ...
	  test_set = bsxfun(@minus,test_set,mn);
test_set = test_set*coefs(:,1:dims); % result is in train_out and test_out
