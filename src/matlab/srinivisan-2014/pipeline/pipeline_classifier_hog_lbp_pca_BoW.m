...
for mm = 1 : size(training_data,1)
	   [knn_idxs D] = knnsearch( C, training_data(mm,:));
temp = hist(knn_idxs,k);
temp_res(mm,:) = temp ./ sum(temp);
    end
    training_data = temp_res;
    
for mm = 1 : size(testing_data,1)
	   [knn_idxs D] = knnsearch( C, testing_data(mm,:));
temp = hist(knn_idxs,k);
temp_res(mm,:) = temp ./ sum(temp);
    end
...
save(strcat(store_directory, 'predicition_hog_lbp_pca_BoW.mat'), 'pred_label_cv');
