function pipeline_classifier_hog_lbp_16_ri_pca(classifier_name)
...
for idx_cv_lpo = 1:length(idx_class_pos)
    ...
    % The two patients for testing will corresspond to the
    % current index of the cross-validation
    ...
    testing_data_hog = [ testing_data_hog ; hog_feat ];
    % Create and concatenate the label
    testing_label = [ testing_label ( -1 * ones(1, size(hog_feat, ...
                                                      1))) ];
    ...
    for tr_idx = 1:length(idx_class_pos)
        ...
        if (tr_idx ~= idx_cv_lpo)
            % Load the positive patient
            load( strcat( data_directory_hog, filename{ ...
                idx_class_pos(tr_idx) ...
                   } ) );
            % Concatenate the data
            training_data_hog = [ training_data_hog ; hog_feat ];
            % Create and concatenate the label
            training_label = [ training_label ones(1, size(hog_feat, ...
                                                           1)) ];
            % Load the negative patient
            load( strcat( data_directory_hog, filename{ ...
                idx_class_neg(tr_idx) } ) );
            % Concatenate the data
            training_data_hog = [ training_data_hog ; hog_feat ];
            % Create and concatenate the label
            training_label = [ training_label (-1 * ones(1, ...
                                                         size(hog_feat, ...
                                                              1)))];
        end
    end
    ...
    testing_data_hog = (bsxfun(@minus, testing_data_hog, mu)) * ...
        coeff;
    ...
    for tr_idx = 1:length(idx_class_pos)
        ...
        if (tr_idx ~= idx_cv_lpo)
            % Load the positive patient
            load( strcat( data_directory_lbp, filename{ ...
                idx_class_pos(tr_idx) } ) );
            % Concatenate the data
    ...
    % Apply the transformation to the testing data
    % Remove the mean computed during the training of the PCA
    testing_data_lbp = (bsxfun(@minus, testing_data_lbp, mu)) * ...
        coeff;
    ...
    save(strcat(store_directory, ['predicition_hog_lbp_16_ri_pca', classifier_name, '.mat']), 'pred_label_cv');
